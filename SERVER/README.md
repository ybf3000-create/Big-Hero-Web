...
created: 2026-09-11
version: v1.0
status: 服务端权威玩法、断线恢复与安全记住登录已接入，待跨机完整验收
---
...
检查覆盖账号与角色输入、20/21人容量、同账号替换会话、30秒断线名额、记住登录签发/轮换/撤销、SQLite摘要存储、注册登录角色主流程、WebSocket聊天、拍卖交易守恒、断线后战斗状态恢复、损坏状态保护，以及服务端战斗和全部200级Boss生成。当前自动测试为50项，并已在Edge中验证Godot画布、HttpOnly Cookie和刷新自动恢复。已有测试不等于跨机验收；后续仍需在另一台ZeroTier电脑完成登录、聊天、拍卖和长期运行验证。
