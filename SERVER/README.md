...
created: 2026-09-11
version: v0.9
status: 服务端权威玩法与安全记住登录已接入，待跨机完整验收
---

# 大勇者服务端 v0.9

本目录是Godot浏览器网络版的单体服务，也是唯一维护的服务端。当前代码覆盖邀请码注册、账号密码登录、安全记住登录、最多20个账号同时在线、同账号新端踢旧端、30秒断线恢复、每账号1个角色、世界聊天和一口价拍卖行。地图、Boss 1～200、装备、宝石、技能、属性点、掉落、扑克奖励、天命卡及离线收益已改为服务端运算并接回Godot网页界面。自动测试和本机浏览器流程已通过，但另一台物理电脑的ZeroTier跨机完整玩法仍需最终验收。桌面原生客户端不再交付或维护。
...
检查覆盖账号与角色输入、20/21人容量、同账号替换会话、30秒断线名额、记住登录签发/轮换/撤销、SQLite摘要存储、注册登录角色主流程、WebSocket聊天、拍卖交易守恒，以及服务端战斗和全部200级Boss生成。当前自动测试为44项，并已在Edge中验证Godot画布、HttpOnly Cookie和刷新自动恢复。已有测试不等于跨机验收；后续仍需在另一台ZeroTier电脑完成登录、聊天、拍卖和长期运行验证。
