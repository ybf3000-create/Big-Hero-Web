...
继续复用`client/`中的原版UI、地图场景、逐格移动、装备、技能和战斗表现，不用`web-client/`中的手写网页原型替代游戏，也不擅自简化玩法、增加道具或改变数值。定案与现有代码不一致、浏览器能力不足或表现不明确时，先指出具体位置并讨论，不能自行换成简化方案。完整边界见本地策划案《网页网络游戏改造总案【定案】》§3.3；`策划案/`目前未上传本仓库。
