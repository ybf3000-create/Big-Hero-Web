extends Node

const UI_FONT: Font = preload("res://assets/fonts/NotoSansHans-Regular.otf")
const SYMBOL_FONT: Font = preload("res://assets/fonts/NotoSansSymbols2-Regular.ttf")
var _ui_font: Font


func _enter_tree() -> void:
	_ui_font = UI_FONT.duplicate()
	# Keep the CJK font authoritative. The symbols fallback is only for card
	# suits and other deliberately retained monochrome symbols; emoji is not a
	# required part of the web UI.
	_ui_font.fallbacks = [SYMBOL_FONT]
	ThemeDB.fallback_font = _ui_font
